...
use Kasi\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->kasi['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->kasi['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->kasi['queue.failer']->forget($id);
            }
        }
    ...
    {
        $this->kasi['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->kasi->bound(Encrypter::class)) {
            $instance = unserialize($this->kasi->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
