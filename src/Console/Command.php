...
use Kasi\Contracts\Console\Isolatable;
...
class Command extends SymfonyCommand
{
    ...
    {
        $this->output = $output instanceof OutputStyle ? $output : $this->kasi->make(
            OutputStyle::class, ['input' => $input, 'output' => $output]
        );

        $this->components = $this->kasi->make(Factory::class, ['output' => $this->output]);
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            return (int) $this->kasi->call([$this, $method]);
     ...
     *
     * @return \Kasi\Console\CommandMutex
     */
    protected function commandIsolationMutex()
    {
        return $this->kasi->bound(CommandMutex::class)
            ? $this->kasi->make(CommandMutex::class)
            : $this->kasi->make(CacheCommandMutex::class);
    }
    ...
    protected function resolveCommand($command)
    {
        if (is_string($command)) {
            ...
            $command = $this->kasi->make($command);
        }
     ...
     * @return void
     *
     * @throws \Kasi\Console\ManuallyFailedException|\Throwable
     */
    ...
    /**
     * Get the Kasi application instance.
     *
    ...
    /**
     * Set the Kasi application instance.
     *
     * @param  \Kasi\Contracts\Container\Container  $kasi
     * @return void
     */
    public function setKasi($kasi)
    {
        $this->kasi = $kasi;
    }
}
