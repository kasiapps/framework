...
use Kasi\Console\PromptValidationException;
...
trait ConfiguresPrompts
{
    ...
    protected function configurePrompts(InputInterface $input)
    {
        ...
        Prompt::interactive(($input->isInteractive() && defined('STDIN') && stream_isatty(STDIN)) || $this->kasi->runningUnitTests());

        Prompt::validateUsing(fn (Prompt $prompt) => $this->validatePrompt($prompt->value(), $prompt->validate));

        Prompt::fallbackWhen(windows_os() || $this->kasi->runningUnitTests());
    ...
    {
        return $this->kasi['validator']->make(
            [$field => $value],
            [$field => $rules],
    ...
    private function multiselectFallback($label, $options, $default = [], $required = false)
    {
        ...
        if ($required === false && ! $this->kasi->runningUnitTests()) {
