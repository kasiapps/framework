...
use Kasi\Http\Client\Events\ConnectionFailed;
...
class PendingRequest
{
     ...
     *
     * @var \Kasi\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \Kasi\Support\Collection|null
     */
     ...
     *
     * @var \Kasi\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \Kasi\Http\Client\Factory|null  $factory
     ...
     * @return \Kasi\Http\Client\Response
     *
     * @throws \Kasi\Http\Client\ConnectionException
     */
     ...
     *
     * @param  callable  $callback
     * @return array<array-key, \Kasi\Http\Client\Response>
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $kasiData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $kasiData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($kasiData) && $method === 'GET' && $urlString->contains('?')) {
            $kasiData = (string) $urlString->after('?');
        }

        if (is_string($kasiData)) {
            parse_str($kasiData, $parsedData);

            $kasiData = is_array($parsedData) ? $parsedData : [];
        }

        if ($kasiData instanceof JsonSerializable) {
            $kasiData = $kasiData->jsonSerialize();
        }

        return is_array($kasiData) ? $kasiData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['kasi_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? new Collection)
                    ->map
                    ->__invoke((new Request($request))->withData($options['kasi_data']), $options)
                    ->filter()
                    ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['kasi_data']), $options, $this
                );
