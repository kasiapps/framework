...
namespace Kasi\Foundation\Concerns;
...
use Kasi\Foundation\Http\Request as KasiRequest;
use Kasi\Foundation\Routing\Closure as RoutingClosure;
use Kasi\Foundation\Routing\Controller as KasiController;
use Kasi\Foundation\Routing\Pipeline;
...
trait RoutesRequests
{
  ...
  protected function callControllerAction($routeInfo)
  {
    ...
    if ($instance instanceof KasiController) {
      return $this->callKasiController($instance, $method, $routeInfo);
    } else {
  ...
  /**
   * Send the request through a Kasi controller.
   *
   ...
   * @return mixed
   */
  protected function callKasiController($instance, $method, $routeInfo)
  {
    $middleware = $instance->getMiddlewareForMethod($method);

    if (count($middleware) > 0) {
      return $this->callKasiControllerWithMiddleware(
   ...
   * @return mixed
   */
  protected function callKasiControllerWithMiddleware($instance, $method, $routeInfo, $middleware)
  {
