...
use Kasi\Cache\Events\KeyWriteFailed;
...
use function Kasi\Support\defer;

/**
 * @mixin \Kasi\Contracts\Cache\Store
 */
class Repository implements ArrayAccess, CacheContract
{
     ...
     *
     * @var \Kasi\Contracts\Cache\Store
     */
    protected $store;
    ...
    public function flexible($key, $ttl, $callback, $lock = null)
    {
        [
            $key => $value,
            "kasi:cache:flexible:created:{$key}" => $created,
        ] = $this->many([$key, "kasi:cache:flexible:created:{$key}"]);

        if (in_array(null, [$value, $created], true)) {
            return tap(value($callback), fn ($value) => $this->putMany([
                $key => $value,
                "kasi:cache:flexible:created:{$key}" => Carbon::now()->getTimestamp(),
            ], $ttl[1]));
        }
        ...
        $refresh = function () use ($key, $ttl, $callback, $lock, $created) {
            $this->store->lock(
                "kasi:cache:flexible:lock:{$key}",
                $lock['seconds'] ?? 0,
                $lock['owner'] ?? null,
            )->get(function () use ($key, $callback, $created, $ttl) {
                if ($created !== $this->get("kasi:cache:flexible:created:{$key}")) {
                    return;
                }
        ...
        defer($refresh, "kasi:cache:flexible:{$key}");
     ...
     *
     * @return \Kasi\Contracts\Cache\Store
     */
     ...
     *
     * @return \Kasi\Contracts\Events\Dispatcher|null
     */
