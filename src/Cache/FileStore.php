...
use Kasi\Contracts\Filesystem\LockTimeoutException;
use Kasi\Filesystem\Filesystem;
use Kasi\Filesystem\LockableFile;
    ...
    public function forget($key)
    {
        if ($this->files->exists($file = $this->path($key))) {
            return tap($this->files->delete($file), function ($forgotten) use ($key) {
                if ($forgotten && $this->files->exists($file = $this->path("kasi:cache:flexible:created:{$key}"))) {
