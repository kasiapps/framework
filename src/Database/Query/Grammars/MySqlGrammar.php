...
namespace Kasi\Database\Query\Grammars;

use Kasi\Database\Query\Builder;
use Kasi\Database\Query\JoinLateralClause;
...
class MySqlGrammar extends Grammar
{
     ...
     *
     * @param  \Kasi\Database\Query\Builder  $query
     * @param  \Kasi\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    protected function compileLegacyGroupLimit(Builder $query)
    {
        ...
        $partition = ', @kasi_row := if(@kasi_group = '.$column.', @kasi_row + 1, 1) as `kasi_row`';
        $partition .= ', @kasi_group := '.$column;
        ...
        $from = '(select @kasi_row := 0, @kasi_group := 0) as `kasi_vars`, ('.$sql.') as `kasi_table`';

        $sql = 'select `kasi_table`.*'.$partition.' from '.$from.' having `kasi_row` <= '.$limit;

        if (isset($offset)) {
            $sql .= ' and `kasi_row` > '.$offset;
        }

        return $sql.' order by `kasi_row`';
    }
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = (new Collection($update))->map(function ($value, $key) use ($useUpsertAlias) {
            ...
            return $useUpsertAlias
                ? $this->wrap($value).' = '.$this->wrap('kasi_upsert_alias').'.'.$this->wrap($value)
     ...
     *
     * @param  \Kasi\Database\Query\JoinLateralClause  $join
