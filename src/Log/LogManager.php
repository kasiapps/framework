...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \Kasi\Log\Logger  $logger
     * @return \Kasi\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/kasi.log',
            $this->level(['level' => 'debug'])
        );

        return new Logger(
            new Monolog('kasi', $this->prepareHandlers([$handler])),
            $this->app['events']
        );
    }
