...
class BladeCompiler extends Compiler implements CompilerInterface
{
     ...
     *
     * @param  \Kasi\View\Component  $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php if (\Kasi\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Kasi\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\Kasi\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Kasi\Support\Facades\Blade::check('{$name}')): ?>";
        });
