...
use Kasi\View\AnonymousComponent;
use Kasi\View\DynamicComponent;
use Kasi\View\ViewFinderInterface;
...
class ComponentTagCompiler
{
     ...
     * @param  \Kasi\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(Kasi\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
     ...
     *
     * @param  \Kasi\Contracts\View\Factory  $viewFactory
     * @param  string  $component
     * @return string|null
     */
        ...
        return preg_replace_callback(
            '/@(class)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'class') {
                    ...
                    return ":class=\"\Kasi\Support\Arr::toCssClasses{$match[2]}\"";
                }
        ...
        return preg_replace_callback(
            '/@(style)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'style') {
                    ...
                    return ":style=\"\Kasi\Support\Arr::toCssStyles{$match[2]}\"";
                }
        ...
        return (new Collection($attributes))
            ->map(function (string $value, string $attribute) use ($escapeBound) {
                return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                            ? "'{$attribute}' => \Kasi\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                            : "'{$attribute}' => {$value}";
            })
            ->implode(',');
    }
