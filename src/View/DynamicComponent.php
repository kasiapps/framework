...
class DynamicComponent extends Component
{
     ...
     *
     * @var \Kasi\View\Compilers\BladeTagCompiler
     */
     ...
     *
     * @return \Kasi\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract((new \Kasi\Support\Collection($attributes->getAttributes()))->mapWithKeys(function ($value, $key) { return [Kasi\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__kasi_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \Kasi\View\Compilers\ComponentTagCompiler
     */
