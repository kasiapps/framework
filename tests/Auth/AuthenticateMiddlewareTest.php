...
use Kasi\Auth\EloquentUserProvider;
use Kasi\Auth\Middleware\Authenticate;
use Kasi\Auth\Middleware\AuthenticateWithBasicAuth;
use Kasi\Auth\RequestGuard;
use Kasi\Config\Repository as Config;
...
class AuthenticateMiddlewareTest extends TestCase
{
    ...
    public function testItCanGenerateDefinitionViaStaticMethod()
    {
        ...
        $this->assertSame('Kasi\Auth\Middleware\Authenticate:foo', $signature);

        $signature = (string) Authenticate::using('foo', 'bar');
        $this->assertSame('Kasi\Auth\Middleware\Authenticate:foo,bar', $signature);

        $signature = (string) Authenticate::using('foo', 'bar', 'baz');
        $this->assertSame('Kasi\Auth\Middleware\Authenticate:foo,bar,baz', $signature);
    }

    public function testItCanGenerateDefinitionViaStaticMethodForBasic()
    {
        $signature = (string) AuthenticateWithBasicAuth::using('guard');
        $this->assertSame('Kasi\Auth\Middleware\AuthenticateWithBasicAuth:guard', $signature);

        $signature = (string) AuthenticateWithBasicAuth::using('guard', 'field');
        $this->assertSame('Kasi\Auth\Middleware\AuthenticateWithBasicAuth:guard,field', $signature);

        $signature = (string) AuthenticateWithBasicAuth::using(field: 'field');
        $this->assertSame('Kasi\Auth\Middleware\AuthenticateWithBasicAuth:,field', $signature);
    }
