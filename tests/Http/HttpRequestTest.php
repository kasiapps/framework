...
use Kasi\Tests\Database\Fixtures\Models\Money\Price;
...
class HttpRequestTest extends TestCase
{
    ...
    {
        $request = Request::create('/', 'GET', [], [], [], [
            'HTTP_USER_AGENT' => 'Kasi',
        ]);

        $this->assertSame('Kasi', $request->userAgent());
    }
    ...
    public function testOldMethodCallsSession()
    {
        ...
        $request->setKasiSession($session);
    ...
    public function testGetSessionMethodWithKasiSession()
    {
        $request = Request::create('/');

        $kasiSession = m::mock(Store::class);
        $request->setKasiSession($kasiSession);

        $session = $request->getSession();
        $this->assertInstanceOf(SessionInterface::class, $session);

        $kasiSession->shouldReceive('start')->once()->andReturn(true);
        $session->start();
    }

    public function testGetSessionMethodWithoutKasiSession()
    {
     ...
     *
     * Form request object attribute returns empty when have some string.
     */
    ...
    public function testItCanHaveObjectsInJsonPayload()
    {
        ...
        $base = SymfonyRequest::create('/', 'POST', server: ['CONTENT_TYPE' => 'application/json'], content: '{"framework":{"name":"Kasi"}}');
        $request = Request::createFromBase($base);

        $value = $request->get('framework');

        $this->assertSame(['name' => 'Kasi'], $request->get('framework'));
    }
