...
use Kasi\Database\Connectors\Connector;
use Kasi\Database\Connectors\MySqlConnector;
use Kasi\Database\Connectors\PostgresConnector;
use Kasi\Database\Connectors\SQLiteConnector;
use Kasi\Database\Connectors\SqlServerConnector;
use Mockery as m;
use PDO;
use PDOStatement;
...
class DatabaseConnectorTest extends TestCase
{
    ...
    {
        $dsn = 'pgsql:host=foo;dbname=\'bar\';client_encoding=\'utf8\';application_name=\'Kasi App\'';
        $config = ['host' => 'foo', 'database' => 'bar', 'charset' => 'utf8', 'application_name' => 'Kasi App'];
        $connector = $this->getMockBuilder(PostgresConnector::class)->onlyMethods(['createConnection', 'getOptions'])->getMock();
        $connection = m::mock(stdClass::class);
        $connector->expects($this->once())->method('getOptions')->with($this->equalTo($config))->willReturn(['options']);
        $connector->expects($this->once())->method('createConnection')->with($this->equalTo($dsn), $this->equalTo($config), $this->equalTo(['options']))->willReturn($connection);
