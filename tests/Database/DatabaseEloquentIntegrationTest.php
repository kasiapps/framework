...
use Kasi\Database\Eloquent\Relations\MorphPivot;
...
use Kasi\Database\QueryException;
...
use Kasi\Tests\Integration\Database\Fixtures\Post;
...
class DatabaseEloquentIntegrationTest extends TestCase
{
    ...
    public function testBasicModelRetrieval()
    {
        ...
        $this->assertTrue(EloquentTestUser::where('email', 'mohamed@kasi.com')->doesntExist());
    ...
    public function testFindOrFailWithSingleIdThrowsModelNotFoundException()
    {
        ...
        $this->expectExceptionMessage('No query results for model [Kasi\Tests\Database\EloquentTestUser] 1');
    ...
    {
        $this->expectException(ModelNotFoundException::class);
        $this->expectExceptionMessage('No query results for model [Kasi\Tests\Database\EloquentTestUser] 2, 3');
        $this->expectExceptionObject(
    ...
    {
        $user = EloquentTestUser::create(['email' => 'taylor@kasi.com']);
        $this->connection()->transaction(function () use ($user) {
            try {
                $this->connection()->transaction(function () use ($user) {
                    $user->email = 'otwell@kasi.com';
                    $user->save();
                    throw new Exception;
                });
            } catch (Exception) {
            ...
            $this->assertSame('taylor@kasi.com', $user->email);
        });
    }
    ...
    public function testNestedTransactionsUsingSaveOrFailWillSucceed()
    {
        ...
        $this->connection()->transaction(function () use ($user) {
            ...
            $user = EloquentTestUser::first();
            $this->assertSame('otwell@kasi.com', $user->email);
    ...
    public function testTimestampsUsingOldSqlServerDateFormatFallbackToDefaultParsing()
    {
        ...
        // No longer throwing exception since Kasi 7,
