...
class DatabaseSqliteSchemaStateTest extends TestCase
{
    ...
    public function testLoadSchemaToDatabase(): void
    {
        ...
        $processFactory->shouldHaveBeenCalled()->with('sqlite3 "${:KASI_LOAD_DATABASE}" < "${:KASI_LOAD_PATH}"');

        $process->shouldHaveReceived('mustRun')->with(null, [
            'KASI_LOAD_DATABASE' => 'database/database.sqlite',
            'KASI_LOAD_PATH' => 'database/schema/sqlite-schema.dump',
        ]);
    }
