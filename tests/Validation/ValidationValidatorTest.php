...
use Kasi\Contracts\Auth\Authenticatable;
use Kasi\Contracts\Auth\Guard;
use Kasi\Contracts\Hashing\Hasher;
use Kasi\Contracts\Translation\Translator as TranslatorContract;
use Kasi\Contracts\Validation\DataAwareRule;
use Kasi\Contracts\Validation\ImplicitRule;
use Kasi\Contracts\Validation\Rule;
use Kasi\Contracts\Validation\ValidatorAwareRule;
use Kasi\Database\Eloquent\Model;
use Kasi\Support\Arr;
use Kasi\Support\Carbon;
use Kasi\Support\Exceptions\MathException;
use Kasi\Support\Stringable;
use Kasi\Translation\ArrayLoader;
use Kasi\Translation\Translator;
use Kasi\Validation\DatabasePresenceVerifierInterface;
use Kasi\Validation\Rule as ValidationRule;
use Kasi\Validation\Rules\Exists;
use Kasi\Validation\Rules\ProhibitedIf;
use Kasi\Validation\Rules\Unique;
use Kasi\Validation\ValidationData;
...
class ValidationValidatorTest extends TestCase
{
    ...
    public function testCustomExceptionMustExtendValidationException()
    {
        ...
        $this->expectExceptionMessage('Exception [RuntimeException] is invalid. It must extend [Kasi\Validation\ValidationException].');
    ...
    public function testValidateEndsWith()
    {
        ...
        $v = new Validator($trans, ['url' => 'kasi.com'], ['url' => 'ends_with:http']);
        $this->assertFalse($v->passes());
        $this->assertSame('The url must end with one of the following values http', $v->messages()->first('url'));

        $trans = $this->getKasiArrayTranslator();
        $trans->addLines(['validation.ends_with' => 'The :attribute must end with one of the following values :values'], 'en');
        $v = new Validator($trans, ['url' => 'kasi.com'], ['url' => 'ends_with:http,https']);
        $this->assertFalse($v->passes());
    ...
    public function testValidateStartsWith()
    {
        ...
        $v = new Validator($trans, ['url' => 'kasi.com'], ['url' => 'starts_with:http']);
        $this->assertFalse($v->passes());
        $this->assertSame('The url must start with one of the following values http', $v->messages()->first('url'));

        $trans = $this->getKasiArrayTranslator();
        $trans->addLines(['validation.starts_with' => 'The :attribute must start with one of the following values :values'], 'en');
        $v = new Validator($trans, ['url' => 'kasi.com'], ['url' => 'starts_with:http,https']);
        $this->assertFalse($v->passes());
    ...
    {
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'foo@gmäil.com'], ['x' => 'email']);
        $this->assertTrue($v->passes());
    }

    public function testValidateEmailWithStrictCheck()
    {
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:strict']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterCheck()
    {
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter']);
        $this->assertTrue($v->passes());

        // Unicode characters are not allowed
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterUnicodeCheck()
    {
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        // Any unicode characters are allowed only in local-part
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateEmailWithCustomClassCheck()
    {
        ...
        $v = new Validator($this->getKasiArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:'.NoRFCWarningsValidation::class]);
        $v->setContainer($container);
    ...
    public function testValidateUrlWithProtocols()
    {
        ...
        $v = new Validator($trans, ['x' => 'http://kasi.com'], ['x' => 'url:https']);
        $this->assertFalse($v->passes());
...
1',
    ...
    public function testItemAwareSometimesAddingRules()
    {
        ...
        $trans = $this->getKasiArrayTranslator();
        $v = new Validator($trans, ['attendee' => ['name' => 'Taylor', 'title' => 'Creator of Kasi', 'type' => 'Developer']], ['attendee.*' => 'string']);
    ...
    public function testValidateReturnsValidatedData()
    {
        ...
        $v = new Validator($this->getKasiArrayTranslator(), $post, ['first' => 'required', 'preferred' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedRules()
    {
        ...
        $v = new Validator($this->getKasiArrayTranslator(), $post, $rules);
        $v->sometimes('type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateReturnsValidatedDataNestedChildRules()
    {
        ...
        $v = new Validator($this->getKasiArrayTranslator(), $post, ['nested.foo' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedArrayRules()
    {
        ...
        $v = new Validator($this->getKasiArrayTranslator(), $post, ['nested.*.bar' => 'required']);
        $v->sometimes('nested.*.type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateFailsWithAsterisksAsDataKeys()
    {
        ...
        $validator = new Validator($this->getKasiArrayTranslator(), $post, $rules);
