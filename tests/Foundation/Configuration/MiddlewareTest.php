...
namespace Kasi\Tests\Foundation\Configuration;

use Kasi\Container\Container;
use Kasi\Contracts\Encryption\Encrypter;
use Kasi\Contracts\Foundation\Application;
use Kasi\Contracts\Foundation\MaintenanceMode;
use Kasi\Cookie\Middleware\EncryptCookies;
use Kasi\Foundation\Configuration\Middleware;
use Kasi\Foundation\Http\Middleware\ConvertEmptyStringsToNull;
use Kasi\Foundation\Http\Middleware\PreventRequestsDuringMaintenance;
use Kasi\Foundation\Http\Middleware\TrimStrings;
use Kasi\Http\Middleware\TrustHosts;
use Kasi\Http\Middleware\TrustProxies;
...
class MiddlewareTest extends TestCase
{
    ...
    public function testTrustHosts()
    {
        ...
        $this->assertEquals(['^(.+\.)?kasi\.test$'], $middleware->hosts());

        $configuration->trustHosts();
        $this->assertEquals(['^(.+\.)?kasi\.test$'], $middleware->hosts());

        $configuration->trustHosts(at: ['my.test']);
        $this->assertEquals(['my.test', '^(.+\.)?kasi\.test$'], $middleware->hosts());
