...
use Kasi\Foundation\Bootstrap\HandleExceptions;
...
class HandleExceptionsTest extends TestCase
{
    ...
    public function testPhpDeprecationsWithStackTraces()
    {
        ...
        $logger->expects('warning')->with(
            m::on(fn (string $message) => (bool) preg_match(
                <<<REGEXP
                #ErrorException: str_contains\(\): Passing null to parameter \#2 \(\\\$needle\) of type string is deprecated in /home/user/kasi/routes/web\.php:17
                Stack trace:
                \#0 .*helpers.php\(.*\): Kasi\\\\Foundation\\\\Bootstrap\\\\HandleExceptions.*
                \#1 .*HandleExceptions\.php\(.*\): with.*
                \#2 .*HandleExceptions\.php\(.*\): Kasi\\\\Foundation\\\\Bootstrap\\\\HandleExceptions->handleDeprecation.*
                \#3 .*HandleExceptionsTest\.php\(.*\): Kasi\\\\Foundation\\\\Bootstrap\\\\HandleExceptions->handleError.*
                [\s\S]*#i
                REGEXP,
                $message
            ))
    ...
    public function testErrors()
    {
        ...
        $this->handleExceptions()->handleError(
            ...
            '/home/user/kasi/src/Providers/AppServiceProvider.php',
            17
        );
    }
