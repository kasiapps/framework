...
namespace Kasi\Tests\Console;

use Kasi\Console\Application;
use Kasi\Console\Command;
use Kasi\Contracts\Events\Dispatcher;
use Kasi\Contracts\Foundation\Application as ApplicationContract;
use Kasi\Events\Dispatcher as EventsDispatcher;
use Kasi\Foundation\Application as FoundationApplication;
use Kasi\Tests\Console\Fixtures\FakeCommandWithArrayInputPrompting;
use Kasi\Tests\Console\Fixtures\FakeCommandWithInputPrompting;
use Mockery as m;
...
class ConsoleApplicationTest extends TestCase
{
    ...
    public function testAddSetsKasiInstance()
    {
        $app = $this->getMockConsole(['addToParent']);
        $command = m::mock(Command::class);
        $command->shouldReceive('setKasi')->once()->with(m::type(ApplicationContract::class));
    ...
    public function testKasiNotSetOnSymfonyCommands()
    {
        $app = $this->getMockConsole(['addToParent']);
        $command = m::mock(SymfonyCommand::class);
        $command->shouldReceive('setKasi')->never();
        $app->expects($this->once())->method('addToParent')->with($this->equalTo($command))->willReturn($command);
        $result = $app->add($command);
    ...
    public function testResolveAddsCommandViaApplicationResolution()
    {
        $app = $this->getMockConsole(['addToParent']);
        $command = m::mock(SymfonyCommand::class);
        $app->getKasi()->shouldReceive('make')->once()->with('foo')->andReturn(m::mock(SymfonyCommand::class));
        $app->expects($this->once())->method('addToParent')->with($this->equalTo($command))->willReturn($command);
    ...
    {
        $app = new Application(
            $app = new \Kasi\Foundation\Application(__DIR__),
            $events = m::mock(Dispatcher::class, ['dispatch' => null, 'fire' => null]),
            'testing'
        );
