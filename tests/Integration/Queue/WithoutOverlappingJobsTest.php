...
use Kasi\Queue\Middleware\WithoutOverlapping;
use Mockery as m;
...
class WithoutOverlappingJobsTest extends QueueTestCase
{
    ...
    public function testGetLock()
    {
        ...
        $this->assertSame(
            'kasi-queue-overlap:Kasi\\Tests\\Integration\\Queue\\OverlappingTestJob:key',
            (new WithoutOverlapping('key'))->getLockKey($job)
        );

        $this->assertSame(
            'kasi-queue-overlap:key',
            (new WithoutOverlapping('key'))->shared()->getLockKey($job)
        );

        $this->assertSame(
            'prefix:Kasi\\Tests\\Integration\\Queue\\OverlappingTestJob:key',
