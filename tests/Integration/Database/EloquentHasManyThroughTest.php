...
namespace Kasi\Tests\Integration\Database\EloquentHasManyThroughTest;
...
class EloquentHasManyThroughTest extends DatabaseTestCase
{
    ...
    public function testBasicCreateAndRetrieve()
    {
        ...
        $this->assertEquals(
            $mate1->refresh()->getAttributes() + ['kasi_through_key' => '1'],
        ...
        $this->assertEquals(
            $mate2->refresh()->getAttributes() + ['kasi_through_key' => '1'],
            $result->getAttributes()
        );
    ...
    public function testGlobalScopeColumns()
    {
        ...
        $this->assertEquals(['id' => 2, 'kasi_through_key' => 1], $teamMates[0]->getAttributes());
    ...
    public function testCreateOrFirstWhenRecordDoesntExist()
    {
        ...
        $this->assertEquals('Kasi Forever', $article->title);
    ...
    public function testCreateOrFirstWhenRecordExists()
    {
        ...
        $this->assertEquals('Kasi Forever', $newArticle->title);
    ...
    public function testCreateOrFirstRegressionIssue()
    {
        ...
        $existingTaylorArticle = $taylor->articles()->create(['title' => 'Kasi Forever']);
        ...
        $this->assertEquals('Kasi Forever', $newArticle->refresh()->title);
        $this->assertTrue($taylor->is($newArticle->user));

        $this->assertSame('Kasi Forever', $existingTaylorArticle->refresh()->title);
    ...
    {
        // On Kasi 10.21.0, a bug was introduced that would update the wrong model when using `updateOrCreate()`,
