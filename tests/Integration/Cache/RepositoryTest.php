...
class RepositoryTest extends TestCase
{
    ...
    public function testStaleWhileRevalidate(): void
    {
        ...
        $this->assertSame(946684800, $cache->get('kasi:cache:flexible:created:foo'));
        ...
        $this->assertSame(946684811, $cache->get('kasi:cache:flexible:created:foo')); // this has been updated!
        ...
        $this->assertSame(946684811, $cache->get('kasi:cache:flexible:created:foo'));
        ...
        $this->assertSame(946684832, $cache->get('kasi:cache:flexible:created:foo'));
        ...
        // deferred callback.
        /** @var Lock */
        $lock = $cache->lock('kasi:cache:flexible:lock:foo');
        ...
        $this->assertSame(946684843, $cache->get('kasi:cache:flexible:created:foo'));
        ...
        $cache->putMany([
            'foo' => 99,
            'kasi:cache:flexible:created:foo' => 946684863,
        ]);
        ...
        $this->assertSame(946684863, $cache->get('kasi:cache:flexible:created:foo'));
    }
    ...
    public function testItImplicitlyClearsTtlKeysFromDatabaseCache()
    {
        ...
        $this->assertTrue($cache->has('kasi:cache:flexible:created:count'));

        $cache->forget('count');

        $this->assertEmpty($cache->getConnection()->table('cache')->get());
        $this->assertTrue($cache->missing('count'));
        $this->assertTrue($cache->missing('kasi:cache:flexible:created:count'));
    ...
    public function testItImplicitlyClearsTtlKeysFromFileDriver()
    {
        ...
        $this->assertFalse($cache->getFilesystem()->exists($cache->path('kasi:cache:flexible:created:count')));
        $this->assertTrue($cache->missing('count'));
        $this->assertTrue($cache->missing('kasi:cache:flexible:created:count'));
