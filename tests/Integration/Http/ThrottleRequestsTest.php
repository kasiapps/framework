...
use Kasi\Cache\RateLimiter;
use Kasi\Cache\RateLimiting\GlobalLimit;
use Kasi\Cache\RateLimiting\Limit;
...
use Kasi\Http\Exceptions\ThrottleRequestsException;
use Kasi\Routing\Exceptions\MissingRateLimiterException;
use Kasi\Routing\Middleware\ThrottleRequests;
...
class ThrottleRequestsTest extends TestCase
{
    ...
    public function testItCanGenerateDefinitionViaStaticMethod()
    {
        ...
        $this->assertSame('Kasi\Routing\Middleware\ThrottleRequests:gold-tier', $signature);

        $signature = (string) ThrottleRequests::with(25);
        $this->assertSame('Kasi\Routing\Middleware\ThrottleRequests:25', $signature);

        $signature = (string) ThrottleRequests::with(25, 2);
        $this->assertSame('Kasi\Routing\Middleware\ThrottleRequests:25,2', $signature);

        $signature = (string) ThrottleRequests::with(25, 2, 'foo');
        $this->assertSame('Kasi\Routing\Middleware\ThrottleRequests:25,2,foo', $signature);
        ...
        $this->assertSame('Kasi\Routing\Middleware\ThrottleRequests:60,1,foo', $signature);
    }
