...
use Kasi\Contracts\Http\Kernel;
use Kasi\Foundation\Validation\ValidatesRequests;
use Kasi\Http\Middleware\HandleCors;
...
class HandleCorsTest extends TestCase
{
    ...
    {
        $this->app['config']->set('cors.allowed_origins', ['*.kasi.com']);

        $crawler = $this->call('OPTIONS', 'api/ping', [], [], [], [
            'HTTP_ORIGIN' => 'http://test.kasi.com',
            'HTTP_ACCESS_CONTROL_REQUEST_METHOD' => 'POST',
        ]);

        $this->assertSame('http://test.kasi.com', $crawler->headers->get('Access-Control-Allow-Origin'));
        $this->assertEquals(204, $crawler->getStatusCode());
    }
    ...
    public function testOriginsWildcardIncludesNestedSubdomains()
    {
        ...
        $crawler = $this->call('OPTIONS', 'api/ping', [], [], [], [
            'HTTP_ORIGIN' => 'http://api.service.test.kasi.com',
            'HTTP_ACCESS_CONTROL_REQUEST_METHOD' => 'POST',
        ]);

        $this->assertSame('http://api.service.test.kasi.com', $crawler->headers->get('Access-Control-Allow-Origin'));
        $this->assertEquals(204, $crawler->getStatusCode());
    }
