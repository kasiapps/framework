...
use Kasi\Routing\Exceptions\InvalidSignatureException;
use Kasi\Routing\Middleware\ValidateSignature;
...
class UrlSigningTest extends TestCase
{
    ...
    public function testItCanGenerateMiddlewareDefinitionViaStaticMethod()
    {
        ...
        $this->assertSame('Kasi\Routing\Middleware\ValidateSignature:relative', $signature);

        $signature = (string) ValidateSignature::absolute();
        $this->assertSame('Kasi\Routing\Middleware\ValidateSignature', $signature);

        $signature = (string) ValidateSignature::relative(['foo', 'bar']);
        $this->assertSame('Kasi\Routing\Middleware\ValidateSignature:relative,foo,bar', $signature);

        $signature = (string) ValidateSignature::absolute(['foo', 'bar']);
        $this->assertSame('Kasi\Routing\Middleware\ValidateSignature:foo,bar', $signature);
    }
