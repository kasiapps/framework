...
define('KASI_START', microtime(true));

require __DIR__.'/../../../autoload.php';

\$app = require_once __DIR__.'/bootstrap/app.php';
\$kernel = \$app->make(Kasi\Contracts\Console\Kernel::class);

// Here is our custom command for the test
class CommandSchedulingTestCommand_{$this->id} extends Kasi\Console\Command
{
    ...
    {
        \$logfile = {$logfile};
        (new Kasi\Filesystem\Filesystem)->append(\$logfile, "handled\\n");
    }
}

// Register command with Kernel
Kasi\Console\Application::starting(function (\$artisan) {
...
Kasi\Foundation\Application::getInstance()
    ->booted(function (\$app) {
        \$app->resolving(Kasi\Console\Scheduling\Schedule::class, function(\$schedule) {
            \$fs = new Kasi\Filesystem\Filesystem;
