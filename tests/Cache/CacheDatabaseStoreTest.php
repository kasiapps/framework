...
class CacheDatabaseStoreTest extends TestCase
{
    ...
    public function testNullIsReturnedAndItemDeletedWhenItemIsExpired()
    {
        ...
        $deleteQuery->shouldReceive('whereIn')->once()->with('key', ['prefixfoo', 'prefixkasi:cache:flexible:created:foo'])->andReturn($deleteQuery);
    ...
    public function testItemsMayBeRemovedFromCache()
    {
        ...
        $table = m::mock(stdClass::class);
        $store->getConnection()->shouldReceive('table')->once()->with('table')->andReturn($table);
        $table->shouldReceive('whereIn')->once()->with('key', ['prefixfoo', 'prefixkasi:cache:flexible:created:foo'])->andReturn($table);
