...
use Kasi\Support\ItemNotFoundException;
use Kasi\Support\LazyCollection;
use Kasi\Support\MultipleItemsFoundException;
...
class SupportCollectionTest extends TestCase
{
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'kasi', null, 'kasi', 'php', null])->duplicates()->all();
        $this->assertSame([2 => 1, 5 => 'kasi', 7 => null], $duplicates);
        ...
        $duplicates = $collection::make([1, '2', ['kasi'], ['kasi'], null, '2'])->duplicates()->all();
        $this->assertSame([3 => ['kasi'], 5 => '2'], $duplicates);

        // works with mix of objects and primitives **excepts numbers**.
        $expected = new Collection(['kasi']);
        $duplicates = $collection::make([new Collection(['kasi']), $expected, $expected, [], '2', '2'])->duplicates()->all();
    ...
    {
        $items = [['framework' => 'vue'], ['framework' => 'kasi'], ['framework' => 'kasi']];
        $duplicates = $collection::make($items)->duplicates('framework')->all();
        $this->assertSame([2 => 'kasi'], $duplicates);
    ...
    {
        $duplicates = $collection::make([1, 2, 1, 'kasi', null, 'kasi', 'php', null])->duplicatesStrict()->all();
        $this->assertSame([2 => 1, 5 => 'kasi', 7 => null], $duplicates);
        ...
        // works with mix of primitives
        $duplicates = $collection::make([1, '2', ['kasi'], ['kasi'], null, '2'])->duplicatesStrict()->all();
        $this->assertSame([3 => ['kasi'], 5 => '2'], $duplicates);

        // works with mix of primitives, objects, and numbers
        $expected = new $collection(['kasi']);
        $duplicates = $collection::make([new $collection(['kasi']), $expected, $expected, [], '2', '2'])->duplicatesStrict()->all();
    ...
    public function testReverse($collection)
    {
        ...
        $data = new $collection(['name' => 'taylor', 'framework' => 'kasi']);
        $reversed = $data->reverse();

        $this->assertSame(['framework' => 'kasi', 'name' => 'taylor'], $reversed->all());
    }
    ...
    {
        $data = new $collection(['name' => 'taylor', 'framework' => 'kasi']);
        $this->assertEquals(['taylor' => 'name', 'kasi' => 'framework'], $data->flip()->toArray());
    }
    ...
    public function testGroupByAttributeWithStringableKey($collection)
    {
        $data = new $collection($payload = [
            ['name' => new Stringable('Kasi'), 'url' => '1'],
            ['name' => new HtmlString('Kasi'), 'url' => '1'],
            ['name' => new class()
            {
        ...
        $this->assertEquals(['Kasi' => [$payload[0], $payload[1]], 'Framework' => [$payload[2]]], $result->toArray());
    ...
    {
        $c = new $collection([1, 2, 3, 4, 5, 2, 5, 'name' => 'taylor', 'framework' => 'kasi']);

        $this->assertEquals(1, $c->before(2));
        $this->assertEquals(1, $c->before('2'));
        $this->assertEquals(5, $c->before('taylor'));
        $this->assertSame('taylor', $c->before('kasi'));
    ...
    {
        $c = new $collection([1, 2, 3, 4, 2, 5, 'name' => 'taylor', 'framework' => 'kasi']);
        ...
        $this->assertEquals('kasi', $c->after('taylor'));

        $this->assertEquals(4, $c->after(function ($value) {
            return $value > 2;
        }));
        $this->assertEquals('kasi', $c->after(function ($value) {
            return ! is_numeric($value);
        }));
    }
    ...
    {
        $c = new $collection(['name' => 'taylor', 'framework' => 'kasi']);
        $this->assertEquals(['name', 'framework'], $c->keys()->all());

        $c = new $collection(['taylor', 'kasi']);
