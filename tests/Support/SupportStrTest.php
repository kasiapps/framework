...
class SupportStrTest extends TestCase
{
    ...
    public function testStringTitle()
    {
        ...
        $this->assertSame('123 Kasi', Str::title('123 kasi'));
        $this->assertSame('❤Kasi', Str::title('❤kasi'));
        $this->assertSame('Kasi ❤', Str::title('kasi ❤'));
        $this->assertSame('Kasi123', Str::title('kasi123'));
        $this->assertSame('Kasi123', Str::title('Kasi123'));
    ...
    public function testStringHeadline()
    {
        ...
        $this->assertSame('Jefferson Costella', Str::headline('jefFErson coSTella'));
        $this->assertSame('Jefferson Costella Uses Kasi', Str::headline('jefferson_costella uses-_Kasi'));
        $this->assertSame('Jefferson Costella Uses Kasi', Str::headline('jefferson_costella uses__Kasi'));

        $this->assertSame('Kasi P H P Framework', Str::headline('kasi_p_h_p_framework'));
        $this->assertSame('Kasi P H P Framework', Str::headline('kasi _p _h _p _framework'));
        $this->assertSame('Kasi Php Framework', Str::headline('kasi_php_framework'));
        $this->assertSame('Kasi Ph P Framework', Str::headline('kasi-phP-framework'));
        $this->assertSame('Kasi Php Framework', Str::headline('kasi  -_-  php   -_-   framework   '));
    ...
    public function testStrBeforeLast(): void
    {
        ...
        $this->assertSame('kasi', Str::beforeLast('kasi framework', ' '));
    ...
    {
        $this->assertSame(' kasi php framework ', Str::deduplicate(' kasi   php  framework '));
    ...
    public function testParseCallback()
    {
        ...
        $this->assertEquals(["Class@anonymous\0/kasi/382.php:8$2ec", 'method'], Str::parseCallback("Class@anonymous\0/kasi/382.php:8$2ec@method"));
        $this->assertEquals(["Class@anonymous\0/kasi/382.php:8$2ec", 'method'], Str::parseCallback("Class@anonymous\0/kasi/382.php:8$2ec@method", 'foo'));
        $this->assertEquals(["Class@anonymous\0/kasi/382.php:8$2ec", 'foo'], Str::parseCallback("Class@anonymous\0/kasi/382.php:8$2ec", 'foo'));
        $this->assertEquals(["Class@anonymous\0/kasi/382.php:8$2ec", null], Str::parseCallback("Class@anonymous\0/kasi/382.php:8$2ec"));
    }
    ...
    {
        $this->assertTrue(Str::isUrl('https://kasi.com'));
    ...
    {
        $this->assertTrue(Str::isMatch('/.*,.*!/', 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch('/^.*$(.*)/', 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch('/kasi/i', 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch('/^(.*(.*(.*)))/', 'Hello, Kasi!'));

        $this->assertFalse(Str::isMatch('/H.o/', 'Hello, Kasi!'));
        $this->assertFalse(Str::isMatch('/^kasi!/i', 'Hello, Kasi!'));
        $this->assertFalse(Str::isMatch('/kasi!(.*)/', 'Hello, Kasi!'));
        $this->assertFalse(Str::isMatch('/^[a-zA-Z,!]+$/', 'Hello, Kasi!'));

        $this->assertTrue(Str::isMatch(['/.*,.*!/', '/H.o/'], 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch(['/^kasi!/i', '/^.*$(.*)/'], 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch(['/kasi/i', '/kasi!(.*)/'], 'Hello, Kasi!'));
        $this->assertTrue(Str::isMatch(['/^[a-zA-Z,!]+$/', '/^(.*(.*(.*)))/'], 'Hello, Kasi!'));
    }

    public function testKebab()
    {
        $this->assertSame('kasi-php-framework', Str::kebab('KasiPhpFramework'));
        $this->assertSame('kasi-php-framework', Str::kebab('Kasi Php Framework'));
        $this->assertSame('kasi❤-php-framework', Str::kebab('Kasi ❤ Php Framework'));
    ...
    {
        $this->assertSame('Kasi is...', Str::limit('Kasi is a free, open source PHP web application framework.', 10));
        $this->assertSame('这是一...', Str::limit('这是一段中文', 6));
        $this->assertSame('Kasi is a...', Str::limit('Kasi is a free, open source PHP web application framework.', 15, preserveWords: true));
    ...
    public function testNumbers()
    {
        ...
        $this->assertSame('', Str::numbers('Kasi!'));

        $arrayValue = ['(555) 123-4567', 'L4r4v3l', 'Kasi!'];
    ...
    {
        $this->assertSame('foo bar kasi', Str::replace('baz', 'kasi', 'foo bar baz'));
        $this->assertSame('foo bar kasi', Str::replace('baz', 'kasi', 'foo bar Baz', false));
    ...
    {
        $this->assertSame('kasi_p_h_p_framework', Str::snake('KasiPHPFramework'));
        $this->assertSame('kasi_php_framework', Str::snake('KasiPhpFramework'));
        $this->assertSame('kasi php framework', Str::snake('KasiPhpFramework', ' '));
        $this->assertSame('kasi_php_framework', Str::snake('Kasi Php Framework'));
        $this->assertSame('kasi_php_framework', Str::snake('Kasi    Php      Framework   '));
        // ensure cache keys don't overlap
        $this->assertSame('kasi__php__framework', Str::snake('KasiPhpFramework', '__'));
        $this->assertSame('kasi_php_framework_', Str::snake('KasiPhpFramework_', '_'));
        $this->assertSame('kasi_php_framework', Str::snake('kasi php Framework'));
        $this->assertSame('kasi_php_frame_work', Str::snake('kasi php FrameWork'));
    ...
    {
        $this->assertSame('kasi php framework', Str::squish(' kasi   php  framework '));
        $this->assertSame('kasi php framework', Str::squish("kasi\t\tphp\n\nframework"));
        $this->assertSame('kasi php framework', Str::squish('
            kasi
            php
            framework
        '));
        ...
        $this->assertSame('だ', Str::squish('   だ    '));
        $this->assertSame('ム', Str::squish('   ム    '));
        $this->assertSame('kasi php framework', Str::squish('kasiㅤㅤㅤphpㅤframework'));
        $this->assertSame('kasi php framework', Str::squish('kasiᅠᅠᅠᅠᅠᅠᅠᅠᅠᅠphpᅠᅠframework'));
    }

    public function testStudly()
    {
        $this->assertSame('KasiPHPFramework', Str::studly('kasi_p_h_p_framework'));
        $this->assertSame('KasiPhpFramework', Str::studly('kasi_php_framework'));
        $this->assertSame('KasiPhPFramework', Str::studly('kasi-phP-framework'));
        $this->assertSame('KasiPhpFramework', Str::studly('kasi  -_-  php   -_-   framework   '));
    ...
    {
        $this->assertSame('KasiPhpFramework', Str::pascal('kasi_php_framework'));
        $this->assertSame('KasiPhpFramework', Str::pascal('kasi-php-framework'));
        $this->assertSame('KasiPhpFramework', Str::pascal('kasi  -_-  php   -_-   framework   '));
    ...
    {
        $this->assertSame('kasiPHPFramework', Str::camel('Kasi_p_h_p_framework'));
        $this->assertSame('kasiPhpFramework', Str::camel('Kasi_php_framework'));
        $this->assertSame('kasiPhPFramework', Str::camel('Kasi-phP-framework'));
        $this->assertSame('kasiPhpFramework', Str::camel('Kasi  -_-  php   -_-   framework   '));
    ...
    {
        $this->assertSame(3, Str::substrCount('kasiPHPFramework', 'a'));
        $this->assertSame(0, Str::substrCount('kasiPHPFramework', 'z'));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'l', 2));
        $this->assertSame(0, Str::substrCount('kasiPHPFramework', 'z', 2));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'k', -1));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'k', -1));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'a', 1, 2));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'a', 1, 2));
        $this->assertSame(3, Str::substrCount('kasiPHPFramework', 'a', 1, -2));
        $this->assertSame(1, Str::substrCount('kasiPHPFramework', 'a', -10, -3));
    }
    ...
    public function testSubstrReplace()
    {
        ...
        $this->assertSame('The Kasi Framework', Str::substrReplace('The Framework', 'Kasi ', 4, 0));
        $this->assertSame('Kasi – The PHP Framework for Web Artisans', Str::substrReplace('Kasi Framework', '– The PHP Framework for Web Artisans', 8));
    }
    ...
    {
        $this->assertSame('kasi', Str::lcfirst('Kasi'));
        $this->assertSame('kasi framework', Str::lcfirst('Kasi framework'));
    ...
    {
        $this->assertSame('Kasi', Str::ucfirst('kasi'));
        $this->assertSame('Kasi framework', Str::ucfirst('kasi framework'));
    ...
    {
        $this->assertSame(['Kasi_p_h_p_framework'], Str::ucsplit('Kasi_p_h_p_framework'));
        $this->assertSame(['Kasi_', 'P_h_p_framework'], Str::ucsplit('Kasi_P_h_p_framework'));
        $this->assertSame(['kasi', 'P', 'H', 'P', 'Framework'], Str::ucsplit('kasiPHPFramework'));
        $this->assertSame(['Kasi-ph', 'P-framework'], Str::ucsplit('Kasi-phP-framework'));
    ...
    public function testWordCount()
    {
        ...
        $this->assertEquals(10, Str::wordCount('Hi, this is my first contribution to the Kasi framework.'));
    ...
    public function testInlineMarkdown()
    {
        ...
        $this->assertSame("<a href=\"https://kasi.com\"><strong>Kasi</strong></a>\n", Str::inlineMarkdown('[**Kasi**](https://kasi.com)'));
    }
    ...
    {
        foreach ([
            ['http://kasi.com', 'http://', 'kasi.com'],
            ['http://-http://', 'http://', '-http://'],
            ['http://kasi.com', 'htp:/', 'http://kasi.com'],
            ['http://kasi.com', 'http://www.', 'http://kasi.com'],
            ['http://kasi.com', '-http://', 'http://kasi.com'],
            ['http://kasi.com', ['https://', 'http://'], 'kasi.com'],
            ['http://www.kasi.com', ['http://', 'www.'], 'www.kasi.com'],
