...
use Kasi\Contracts\Validation\Factory;
...
class FullApplicationTest extends TestCase
{
    ...
    public function testGlobalMiddleware()
    {
        ...
        $app->middleware(['KasiTestMiddleware']);
    ...
    public function testRouteMiddleware()
    {
        ...
        $app->routeMiddleware(['foo' => 'KasiTestMiddleware', 'passing' => 'KasiTestPlainMiddleware']);
    ...
    public function testGlobalMiddlewareParameters()
    {
        ...
        $app->middleware(['KasiTestParameterizedMiddleware:foo,bar']);
    ...
    public function testRouteMiddlewareParameters()
    {
        ...
        $app->routeMiddleware(['foo' => 'KasiTestParameterizedMiddleware', 'passing' => 'KasiTestPlainMiddleware']);
    ...
    public function testTerminableGlobalMiddleware()
    {
        ...
        $app->middleware(['KasiTestTerminateMiddleware']);
    ...
    {
        $app = new Application;
        $app->instance(ExceptionHandler::class, $mock = m::mock('Kasi\Foundation\Exceptions\Handler[report]'));
    ...
    {
        $app = new Application;
        $app->instance('request', Request::create('http://kasiapp.com', 'GET'));
        ...
        $this->assertEquals('http://kasiapp.com/something', url('something'));
        $this->assertEquals('http://kasiapp.com/foo-bar', route('foo'));
        $this->assertEquals('http://kasiapp.com/foo-bar/1/2', route('bar', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar?baz=1&boom=2', route('foo', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/1/2', route('optional', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/1', route('optional', ['baz' => 1]));
        $this->assertEquals('http://kasiapp.com/foo-bar/1/2', route('regex', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/1', route('regex', ['baz' => 1]));
    }
    ...
    public function testGeneratingUrlsForRegexParameters()
    {
        ...
        $this->assertEquals('http://kasiapp.com/foo-bar', route('foo'));
        $this->assertEquals('http://kasiapp.com/foo-bar/1/2', route('bar', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/1/2', route('baz', ['baz' => 1, 'boom' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/{baz:[0-9]+}/{boom:[0-9]+}?ba=1&bo=2', route('baz', ['ba' => 1, 'bo' => 2]));
        $this->assertEquals('http://kasiapp.com/foo-bar/5', route('boom', ['baz' => 5]));
    }

    public function testRegisterServiceProvider()
    {
        $app = new Application;
        $provider = new KasiTestServiceProvider($app);
    ...
    public function testApplicationBootsServiceProvidersOnBoot()
    {
        ...
        $provider = new KasiBootableTestServiceProvider($app);
        $app->register($provider);
    ...
    {
        $app = new Application();
        $provider = new class($app) extends \Kasi\Support\ServiceProvider
        {
            public $bootCount = 0;
    ...
    public function testMiddlewareReceiveResponsesEvenWhenStringReturned()
    {
        ...
        $app->routeMiddleware(['foo' => 'KasiTestPlainMiddleware']);
    ...
    public function testBasicControllerDispatching()
    {
        ...
        $app->router->get('/show/{id}', 'KasiTestController@show');
    ...
    {
        $app = new Application;
        $app->routeMiddleware(['test' => KasiTestMiddleware::class]);

        $app->router->group(['middleware' => 'test'], function ($router) {
            $router->get('/show/{id}', 'KasiTestController@show');
        });
    ...
    public function testBasicControllerDispatchingWithMiddlewareIntercept()
    {
        $app = new Application;
        $app->routeMiddleware(['test' => KasiTestMiddleware::class]);
        $app->router->get('/show/{id}', 'KasiTestControllerWithMiddleware@show');
    ...
    public function testBasicInvokableActionDispatching()
    {
        ...
        $app->router->get('/action/{id}', 'KasiTestAction');
    ...
    public function testValidationHelpers()
    {
        ...
        $app->router->get('/', function (Kasi\Http\Request $request) {
    ...
    public function testRedirectToNamedRoute()
    {
        ...
        $app->router->get('login', ['as' => 'login', function (Kasi\Http\Request $request) {
            return 'login';
        }]);
    ...
    public function testRequestUser()
    {
        ...
        $app['auth']->viaRequest('api', function ($request) {
            return new \Kasi\Auth\GenericUser(['id' => 1234]);
        });
    ...
    public function testCanResolveFilesystemFactoryFromContract()
    {
        ...
        $filesystem = $app[Kasi\Contracts\Filesystem\Factory::class];

        $this->assertInstanceOf(Kasi\Contracts\Filesystem\Factory::class, $filesystem);
    }
    ...
    public function testNestedGroupMiddlewaresRequest()
    {
        ...
        $app->router->group(['middleware' => 'middleware1'], function ($router) {
            $router->group(['middleware' => 'middleware2|middleware3'], function ($router) {
                $router->get('test', 'KasiTestController@show');
            });
        });
    ...
    public function testContainerBindingsAreNotOverwritten()
    {
        ...
        $mock = m::mock(Kasi\Bus\Dispatcher::class);

        $app->instance(Kasi\Contracts\Bus\Dispatcher::class, $mock);

        $this->assertSame(
            $mock,
            $app->make(Kasi\Contracts\Bus\Dispatcher::class)
        );
    }

    public function testApplicationClassCanBeOverwritten()
    {
        $app = new KasiTestApplication();

        $this->assertInstanceOf(KasiTestApplication::class, $app->make(Application::class));
    }
    ...
    public function testHandlingCommandsTerminatesApplication()
    {
        ...
        $kernel = $app[Kasi\Foundation\Console\Kernel::class];
...
class KasiTestService
{
}

class KasiTestServiceProvider extends Kasi\Support\ServiceProvider
{
...
class KasiBootableTestServiceProvider extends Kasi\Support\ServiceProvider
{
    public $booted = false;
...
class KasiTestController
{
    public function __construct(KasiTestService $service)
    {
        //
    }
...
class KasiTestControllerWithMiddleware extends Kasi\Foundation\Routing\Controller
{
    public function __construct(KasiTestService $service)
    {
...
class KasiTestParameterizedMiddleware
{
...
class KasiTestApplication extends Application
{
    public function version()
    {
        return 'Custom Kasi App';
    }
}
...
class KasiTestTerminateMiddleware
{
    public function handle($request, $next)
    {
        return $next($request);
...
class ResponsableResponse implements \Kasi\Contracts\Support\Responsable
{
